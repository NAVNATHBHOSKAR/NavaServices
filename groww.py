...
class Current_Holdings_Groww:
    def __init__(self,root):
        ...
        #=================Title======================
        lbl_title=Label(self.root,text="Groww Dashboard", font=("times new roman",16,"bold"),bg="white",fg="royalblue",bd=4,relief=RIDGE)
        ...
        labelframeleft.place(x=5,y=40,width=380,height=280)#width=400 y=35 height=440
        ...
        txtref=ttk.Entry(labelframeleft,width=25,textvariable=self.var_serial_no,font=("arial",11,"bold"))
        ...
        txtname=ttk.Entry(labelframeleft,width=25,textvariable=self.var_company_name,font=("arial",11,"bold"))
        ...
        txtmob=ttk.Entry(labelframeleft,width=25,textvariable=self.var_shares,font=("arial",11,"bold"))
        ...
        txtemail=ttk.Entry(labelframeleft,width=25,textvariable=self.var_avg_price,font=("arial",11,"bold"))
        ...
        txtid=ttk.Entry(labelframeleft,width=25,textvariable=self.var_investment,font=("arial",11,"bold"))
        ...
        btn_frame.place(x=0,y=200,width=370,height=50) #height=25

        btn_add=Button(btn_frame,text="Buy",command=self.add_stocks,font=("arial",11),bg="green",fg="white",width=9) #,"bold"
        ...
        btn_delete=Button(btn_frame,text="Sell",command=self.Sell,font=("arial",11),bg="red",fg="white",width=9) #,"bold"
        btn_delete.grid(row=0,column=2,padx=1)#command=self.Delete,Sell
        ...
        #====================right side img=============================
        img1=Image.open(r"C:\Users\navna\Music\Nava Services\investing.jpg")
        img1=img1.resize((1000,450),Image.ANTIALIAS)
        self.photoimg1=ImageTk.PhotoImage(img1)

        lblimg=Label(self.root,image=self.photoimg1,bd=0,relief=RIDGE)
        lblimg.place(x=0,y=320,width=1000,height=450)

        img2=Image.open(r"C:\Users\navna\Music\Nava Services\unnamed.webp")
        img2=img2.resize((190,190),Image.ANTIALIAS)
        self.photoimg2=ImageTk.PhotoImage(img2)

        lblimg=Label(self.root,image=self.photoimg2,bd=0,relief=RIDGE)
        lblimg.place(x=1000,y=320,width=190,height=190)

        img3=Image.open(r"C:\Users\navna\Music\Nava Services\profile_vrt_raw.jpg")
        img3=img3.resize((190,190),Image.ANTIALIAS)
        self.photoimg3=ImageTk.PhotoImage(img3)

        lblimg=Label(self.root,image=self.photoimg3,bd=0,relief=RIDGE)
        lblimg.place(x=1000,y=510,width=190,height=190)
        
        #=================holdings table frame============================

        Table_frame=LabelFrame(self.root,bd=2,relief=RIDGE,text="Groww Holding Details And Search System",font=("times new roman",12,"bold"),padx=2)
        ...
        btn_search=Button(Table_frame,text="Search",command=self.search,font=("arial",10,"bold"),bg="green",fg="white",width=18)
        btn_search.grid(row=0,column=3,padx=1)#command=self.search,

        btn_Showall=Button(Table_frame,text="Show All",command=self.fetch_data,font=("arial",10,"bold"),bg="dark orange",fg="white",width=18)
...
######
    def add_stocks(self):
        if self.var_company_name.get()=="" or self.var_shares.get()=="":
            ...
            try:
                ...
                my_cursor.execute("insert into groww values(%s,%s,%s,%s,%s)",(
    ...
    def fetch_data(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="MySQL$2022",database="nava_services")
        my_cursor=conn.cursor()
        my_cursor.execute("select * from groww")
    ...
    def update(self):
        if self.var_serial_no.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="MySQL$2022",database="nava_services")
            my_cursor=conn.cursor()
            my_cursor.execute("update groww set Company=%s,Shares=%s,AvgPrice=%s,Investment=%s where Serial=%s",(
                                                                                                                    self.var_company_name.get(),
                                                                                                                    self.var_shares.get(),
                                                                                                                    self.var_avg_price.get(),
                                                                                                                    self.var_investment.get(),
    ...
    def Sell(self):
        ...
        if Sell>0:
            conn=mysql.connector.connect(host="localhost",username="root",password="MySQL$2022",database="nava_services")
            my_cursor=conn.cursor()
            query="delete from groww where Serial=%s"
    ...
    def search(self):
        ...
        my_cursor.execute("select * from groww where "+str(self.search_var.get())+" LIKE '%"+str(self.txt_search.get())+"%'")
        rows=my_cursor.fetchall()
        if len (rows)!=0:
            self.Holding_Details_Table.delete(*self.Holding_Details_Table.get_children())
            for i in rows:
...
if __name__ == "__main__":
    root=Tk()
    obj=Current_Holdings_Groww(root)
    root.mainloop()