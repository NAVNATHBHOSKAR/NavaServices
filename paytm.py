...
class Current_Holdings_Paytm:
    def __init__(self,root):
        ...
        img1=Image.open(r"C:\Users\navna\Music\Nava Services\paradeplatz2.jpg")
        ...
        img2=Image.open(r"C:\Users\navna\Music\Nava Services\paytm.jpg")
        img2=img2.resize((470,400),Image.ANTIALIAS)
        self.photoimg2=ImageTk.PhotoImage(img2)

        lblimg=Label(self.root,image=self.photoimg2,bd=0,relief=RIDGE)
        lblimg.place(x=880,y=310,width=470,height=400)

        #=================Title======================

        lbl_title=Label(self.root,text="Paytm Dashboard", font=("times new roman",16,"bold"),bg="white",fg="royalblue",bd=4,relief=RIDGE)
        ...
        labelframeleft.place(x=880,y=42,width=440,height=265)#width=400 y=35 height=440
        ...
        btn_frame.place(x=0,y=200,width=410,height=40) #height=25
        ...
        Table_frame=LabelFrame(self.root,bd=2,relief=RIDGE,text="Paytm Holding Details And Search System",font=("times new roman",12,"bold"),padx=2)
    ...
    def add_stocks(self):
        if self.var_company_name.get()=="" or self.var_shares.get()=="":
            ...
            try:
                ...
                my_cursor.execute("insert into paytm values(%s,%s,%s,%s,%s)",(
    ...
    def fetch_data(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="MySQL$2022",database="nava_services")
        my_cursor=conn.cursor()
        my_cursor.execute("select * from paytm")
    ...
    def update(self):
        if self.var_serial_no.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="MySQL$2022",database="nava_services")
            my_cursor=conn.cursor()
            my_cursor.execute("update paytm set Company=%s,Shares=%s,AvgPrice=%s,Investment=%s where Serial=%s",(
                                                                                                                    self.var_company_name.get(),
                                                                                                                    self.var_shares.get(),
                                                                                                                    self.var_avg_price.get(),
                                                                                                                    self.var_investment.get(),
    ...
    def Sell(self):
        ...
        if Sell>0:
            conn=mysql.connector.connect(host="localhost",username="root",password="MySQL$2022",database="nava_services")
            my_cursor=conn.cursor()
            query="delete from paytm where Serial=%s"
    ...
    def search(self):
        ...
        my_cursor.execute("select * from paytm where "+str(self.search_var.get())+" LIKE '%"+str(self.txt_search.get())+"%'")
        rows=my_cursor.fetchall()
        if len (rows)!=0:
            self.Holding_Details_Table.delete(*self.Holding_Details_Table.get_children())
            for i in rows:
    ...
    def total(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="MySQL$2022",database="nava_services")
        my_cursor=conn.cursor()
        my_cursor.execute("select round(SUM(Investment),2) from paytm")
...
if __name__ == "__main__":
    root=Tk()
    obj=Current_Holdings_Paytm(root)
    root.mainloop()