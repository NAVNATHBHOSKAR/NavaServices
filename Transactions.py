...
# Display the chart in Streamlit
        ...
        # number of times I have purchased and sold each share
        st.subheader("The number of times I have purchased and sold each share")
        ...
        st.subheader("The valuation of Buying Price, Selling Price & Profit valuation")

        cursor.execute('''
        SELECT share_name,
       SUM(CASE WHEN order_type = 'buy' THEN price_per_share * shares ELSE 0 END) AS total_buying_price,
       SUM(CASE WHEN order_type = 'sell' THEN price_per_share * shares ELSE 0 END) AS total_selling_price,
       SUM(CASE WHEN order_type = 'sell' THEN price_per_share * shares ELSE 0 END) - SUM(CASE WHEN order_type = 'buy' THEN price_per_share * shares ELSE 0 END) AS profit
FROM groww
GROUP BY share_name
ORDER BY share_name
        ''')
        ...
        dfbuysellprofitprice = pd.DataFrame(data, columns=[desc[0] for desc in cursor.description])

        #print(df.columns)
        print(dfbuysellprofitprice.columns)

        # Display the DataFrame
        st.dataframe(dfbuysellprofitprice)

        # Create a bar chart using Plotly
        fig = px.bar(dfbuysellprofitprice, x='share_name', y=['total_buying_price', 'total_selling_price', 'profit'], barmode='group')
        ...
        go.Bar(name='Buying Price', x=dfbuysellprofitprice['share_name'], y=dfbuysellprofitprice['total_buying_price'],
           text=dfbuysellprofitprice['total_buying_price'], textposition='auto'),

        go.Bar(name='Selling Price', x=dfbuysellprofitprice['share_name'],
           y=dfbuysellprofitprice['total_selling_price'],
           text=dfbuysellprofitprice['total_selling_price'], textposition='auto'),

        go.Bar(name='Profit', x=dfbuysellprofitprice['share_name'],
           y=dfbuysellprofitprice['profit'],
           text=dfbuysellprofitprice['profit'], textposition='auto')
        ])

        figure.update_layout(barmode='group', title='Valuation of Buying, Selling & Profit in Rs.',
                     xaxis_title='Share Name', yaxis_title='Valuation in Rs.')
        ...
        # update the chart layout with a proper title
        fig.update_layout(title='Transaction Distribution of Shares', title_font_size=24)
        ...
        # update the chart layout with a proper title
        fig.update_layout(title='Distribution of Order Types', title_font_size=24)

        # display the Plotly chart in the Streamlit app
        st.plotly_chart(fig)

        # Calculate the counts of each order type
        counts = df['order_type'].value_counts()
        ...
        # Create the chart with the filtered DataFrame and set the bar color to green
        chart1 = alt.Chart(filtered_df).mark_bar(color='green').encode(
        x=alt.X('share_name:N', axis=alt.Axis(title='Share Name')),
        y=alt.Y('sum(total_price):Q', axis=alt.Axis(title='Buying Price')),
        tooltip=[
        alt.Tooltip('share_name:N', title='Share Name'),
        alt.Tooltip('sum(total_price):Q', title='Buying Price')
    ]
        ).properties(
        title='Total Buying Price by Share Name'
        )
        ...
        # Filter the DataFrame based on the condition
        filtered_df = df[df['order_type'] == 'sell']

        # Create the chart with the filtered DataFrame and set the bar color to green
        chart2 = alt.Chart(filtered_df).mark_bar(color='red').encode(
        x=alt.X('share_name:N', axis=alt.Axis(title='Share Name')),
        y=alt.Y('sum(total_price):Q', axis=alt.Axis(title='Selling Price')),
        tooltip=[
        alt.Tooltip('share_name:N', title='Share Name'),
        alt.Tooltip('sum(total_price):Q', title='Selling Price')
    ]
        ).properties(
        title='Total Selling Price by Share Name'
        )
    ...
    y='total_price',
    color='share_name',
    labels={'share_name': 'Share Name', 'total_price': 'Buying Price'},
    title='Total Buying Price by Share Name'
)
    ...
    y='total_price',
    color='share_name',
    labels={'share_name': 'Share Name', 'total_price': 'Selling Price'},
    title='Total Selling Price by Share Name'
)
        ...
        with st.form(key='columns_in_form'):
            ...
            with c1:
                # Create a form to collect data from the user
                share_name = st.selectbox("Enter the share name", ["Bajaj Finserv", "Bandhan Bank", "GAIL","Indian Oil Corporation","Indraprastha Gas","Mahindra & Mahindra","Motherson International","NMDC","NTPC","Orient Cement","Power Finance Corporation","Quick Heal Technologies","REC Limited","Steel Authority of India","Tata Coffee","Tata Steel","The Federal Bank","The Indian Hotels Company","Welspun India"])
                price_per_share = st.number_input("Enter the price per share", min_value=0.0)
                shares = st.number_input("Enter the number of shares", min_value=0)
                total_price = price_per_share * shares
        ...
        data = cursor.fetchall()
        st.subheader("View Unique Shares & it's Number of Transactions")
        #Convert the rows to a pandas DataFrame
        dfunique = pd.DataFrame(data, columns=[desc[0] for desc in cursor.description])
...
# Define the heatmap layout and data
        fig = go.Figure(data=go.Heatmap(z=z, colorscale='Viridis'))

# Update the heatmap layout
        fig.update_layout(title='Heatmap Example', xaxis_title='X-axis Label', yaxis_title='Y-axis Label')
